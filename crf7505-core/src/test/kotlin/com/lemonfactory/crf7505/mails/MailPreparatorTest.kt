...
import com.lemonfactory.crf7505.mails.recap.*
...
class MailPreparatorTest {
    private val bodyTemplate = Mockito.mock(RecapBodyTemplate::class.java)
    private val headerTemplate = Mockito.mock(RecapHeaderTemplate::class.java)
    private val footerTemplate = Mockito.mock(RecapFooterTemplate::class.java)
    private val config = Mockito.mock(Config::class.java)

    private val mailPreparator = RecapMailPreparator(bodyTemplate, headerTemplate, footerTemplate, config)
    ...
    fun `should return header body and footer concated`() {
        ...
        // When
        val crfMail = mailPreparator.generateMail(RecapMailParameters(volunteer, Activities(emptyList(), emptyList(), ""), "subject", "header", "footer", "respMission"))
    ...
    fun `should return header body-with-missions and footer concated`() {
        ...
        // When
        val crfMail = mailPreparator.generateMail(RecapMailParameters(volunteer, Activities(listOf(mission), listOf(), ""), "subject", "header", "footer", "respMission"))
